...
//    private Map<Long, Lock> waitLock;   // 正在等待资源的XID的锁
    private Map<Long, CountDownLatch> waitLock;
    private Map<Long, Long> waitU;      // XID正在等待的UID
    private Lock lock;

    public Long lastXid; // 如果发生资源等待，说明是由这个事务释放资源后，就轮到当前事务了。
    ...
    public CountDownLatch add(long xid, long uid) throws Exception {
        ...
        try {
            ...
            if(hasDeadLock()) {
                ...
                throw new RuntimeException("存在死锁");
            }

            CountDownLatch latch = new CountDownLatch(1);
            waitLock.put(xid, latch);
            return latch;
        } finally {
            lock.unlock();
        }
        ...
        while(l.size() > 0) {
            ...
            if(!waitLock.containsKey(xid)) {
               ...
               // Lock lo = waitLock.remove(xid);
                CountDownLatch latch = waitLock.remove(xid);
                waitU.remove(xid);
               // lo.unlock();
                lastXid = orgXid;
                latch.countDown();
